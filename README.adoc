...
One beneift of using build tools is that after you define the details of the project and any dependencies it has, the build tool automatically downloads and installs the dependencies. 
Build tools let you run repeatable, automated tests on the application.
This is better than manually running tests because you can easily rerun the same tests each time the application is built.
If the tests don't pass after you've made a change, the build fails so you know you need to fix your code.

Gradle's benefits are its use of a build cache. 
Whenever the application is rebuilt, only the parts that were changed are rebuilt. 
This reduces build times and increases performance, especially in large projects. 
