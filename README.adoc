...
The [hotspot=27-32 file=1]`dependencies` section defines what is needed to compile and 
...
The [hotspot=41-53 file=1]`liberty` plug-in provides the [hotspot=31 file=1]`libertyRuntime` 
...
The Gradle [hotspot=34-39 file=1]`extra properties` extension allows you to add properties to a Gradle project.
...
The [hotspot=41-53 file=1]`liberty` extension configures the Liberty server, which includes the following settings:
[cols="40, 100"]
|===
| *Server attribute* | *Purpose*
| [hotspot=43 file=1]`name` | The server name.
| [hotspot=44 file=1]`configFile` | The server configuration file.
| [hotspot=45-47 file=1]`bootstrapProperties` | Bootstrap properties that are written to 
                                                 the server's `bootstrap.properties` file.
| [hotspot=48-51 file=1]`packageLiberty` | Specifies to only package the `usr` directory 
                                           in the `build/GradleSample.zip` file.
|===

So what's not configured for the [hotspot=41-53 file=1]`liberty` plug-in?
By default, when the [hotspot=41-53 file=1]`liberty` and the [hotspot=55-57 file=1]`war` 
...
The [hotspot=55-57 file=1]`war` configuration names the WAR file `GradleSample.war` without a version in the name.

== Testing the web application

One of the benefits of building an application with a build system like Gradle is that 
it can be configured to run a set of automated tests. The [hotspot=55-57 file=1]`war` 
...
The [hotspot=59-74 file=1]`test` section in your build file configures Gradle to run the integration tests in the `it` folder.

The [hotspot=73 file=1]`systemProperties` configuration defines some variables needed by 
...
The [hotspot=73 file=1]`systemProperties` section passes these details to the Java test 
program as a series of system properties, resolving the [hotspot=73 file=1]`liberty.test.port` 
and [hotspot=73 file=1]`war.name` variables.
...
== A few more pieces
We show a few more Gradle tricks in this example with the [hotspot=76-82]`openBrowser` and 
the [hotspot=84-89]`openTestReport` tasks. These tasks display your application and the 
...
The [hotspot=91-96]`dependency directives` associate the `war` and `java` tasks to the `liberty` tasks to get proper 
task ordering.
