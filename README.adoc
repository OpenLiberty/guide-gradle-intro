...
automated test, and configure Gradle to run it after building the application. The Gradle Wrapper 
is a script that invokes a declared version of Gradle, downloading it beforehand if necessary. 
The Wrapper is also the recommended way to execute any Gradle build. 
...
If you do not have `gradle` installed, running the gradle wrapper will install gradle. The Gradle 
Wrapper files are included in the project directory. Learn more about Gradle Wrapper on the 
https://docs.gradle.org/current/userguide/gradle_wrapper.html[Official website^]."
