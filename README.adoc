...
Using a build tool like Gradle is beneficial as you can define the details of the projects and its dependencies.
The build tool will automatically handle downloading and installing any and all of the dependencies.
Build tools also allow for defining automated tests that can be easily re-run on each build. 
If the tests don't pass after changes are made, the build fails so you know you need to fix your code. 

Choosing a build tool often comes down to personal preference, but Gradle has some benefits. 
Gradle defines its builds using https://docs.gradle.org/current/userguide/writing_build_scripts.html[Groovy build scripts^]. 
This allows individual developers a lot of control and customization in their builds. 
Gradle also makes use of a build cache, only rebuilding parts that are changed.
This reduces build times and increases performance in larger projects.
This makes Gradle a good choice in larger, more complex projects. 
