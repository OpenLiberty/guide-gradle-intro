...
The first section of code defines the [hotspot=1 file=1]`war` [hotspot=2 file=1]`liberty` plug-ins 
...
The [hotspot=56-58]`war` configuration names the WAR file `GradleSample.war` without a version in the name.
...
The [hotspot=60-75]`test` section in your build file configures Gradle to run the integration tests in the `\*/it/**` folder.
...
The [hotspot=82-97]`dependency directives` associate the `war` and `java` tasks to the `liberty` tasks to get proper 
task ordering.
