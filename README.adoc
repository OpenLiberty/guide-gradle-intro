...
This code snippet defines the [hotspot=1 file=1]`war` [hotspot=2 file=1]`liberty` plug-ins 
that you want to use. The [hotspot=1 file=1]`war` plug-in contains all the tasks to compile 
Java files, build the WAR file structure, and assemble the archive. The [hotspot=2 file=1]`liberty` 
plug-in contains the tasks used to install the Liberty runtime and create and manage 
servers. The [hotspot=4 file=1]`group`, [hotspot=5 file=1]`version`, and [hotspot=6 file=1]`description` 
...
[hotspot=1-29 file=2]`src/main/java/io/openliberty/guides/hello/HelloServlet.java` file 
...
program as a series of system properties, resolving the [hotspot=74 file=1]`liberty.test.port` 
