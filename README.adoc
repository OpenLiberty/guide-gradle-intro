...
This code snippet defines the https://docs.gradle.org/current/userguide/war_plugin.html[`war`] 
and https://github.com/WASdev/ci.gradle/blob/master/README.md[`liberty`] plug-ins that 
you want to use. The `war` plug-in contains all the tasks to compile Java files, build 
the WAR file structure, and assemble the archive. The `liberty` plug-in contains the tasks 
used to install the Liberty runtime and create and manage servers. The `group`, `version`, 
and `description` define this sample project. The compatibility and encoding settings 
are for Java.
...
The [hotspot=14-21 file=1]`buildscript` section defines plug-in versions to use in the 
build and where to find them. This guide uses the [hotspot=19 file=1]`liberty` plug-in, 
...
The [hotspot=23-25 file=1]`repositories` section defines where to find the dependencies 
...
[hotspot=28 file=1]`providedCompile` dependencies are APIs that are needed to compile the 
application, but they do not need to be packaged with the application because Open Liberty 
provides their implementation at run time. For example, the `HelloServlet.java` class 
depends on [hotspot=28 file=1]`javax.servlet-api` to compile, but since Open Liberty 
provides that API, it does not need to be packaged with your application. The 
[hotspot=29-30 file=1]`testCompile` dependencies are needed to compile and run tests.

The [hotspot=42-54 file=1]`liberty` plug-in provides the [hotspot=31-32 file=1]`libertyRuntime` 
...
| [hotspot=46-48 file=1]`bootstrapProperties` | Bootstrap properties that are written to 
                                                 the server's `bootstrap.properties` file.
| [hotspot=49-52 file=1]`packageLiberty` | Specifies to only package the `usr` directory 
                                           in the `build/GradleSample.zip` file.
|===

So what's not configured for the [hotspot=42-54 file=1]`liberty` plug-in?
By default, when the [hotspot=42-54 file=1]`liberty` and the [hotspot=56-58 file=1]`war` 
...
it can be configured to run a set of automated tests. The [hotspot=56-58 file=1]`war` 
...
The [hotspot=74 file=1]`systemProperties` configuration defines some variables needed by 
...
[hotspot=1-97 file=1]`build.gradle` file, in case they need to change.
The [hotspot=74 file=1]`systemProperties` section passes these details to the Java test 
program as a series of system properties, resolving the [hotspot=74 file=1]`liberty.text.port` 
and [hotspot=74 file=1]`war.name` variables.

The following lines in the [hotspot=15-17 file=0]`EndpointIT.java` test class uses these 
...
In the [hotspot=24-33 file=0]`try` block of the test method, an HTTP `GET` request to the 
...
test has some new dependencies. Earlier you added some [hotspot=29-30 file=1]`testCompile` 
dependencies. The Apache [hotspot=29 file=1]`commons-httpclient` and [hotspot=30 file=1]`junit` 
dependencies are needed to compile and run the integration test [hotspot=10-35 file=0]`EndpointIT` 
class.

The scope for each of the dependencies is set to [hotspot=29-30 file=1]`testCompile` 
...
== A few more pieces
We show a few more Gradle tricks in this example with the [hotspot=77-83]`openBrowser` and 
the [hotspot=85-90]`openTestReport` tasks. These tasks display your application and the 
...
These statements associate the `war` and `java` tasks to the `liberty` tasks to get proper 
task ordering.
...
From the folder containing your [hotspot=1-97 file=0]`build.gradle` file, run the command:
...
[hotspot=1-29 file=1]`src/main/java/io/openliberty/guides/hello/HelloServlet.java` so 
that it doesn't match the string that the test is looking for. Then rerun the Gradle 
integration tests by running the command:

HelloServlet.java   
[source, Java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/hello/HelloServlet.java[tag=**;!comment]
----
...
You can quickly create this project structure by using the https://liberty-app-accelerator.wasdev.developer.ibm.com/start/[Liberty App Accelerator] 
and choose to create a Gradle project.
