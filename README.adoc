...
[role="code_command hotspot=14-21", file=1, subs="quotes"] 
---- 
Add the `buildscript` section to the build file.
`/build.gradle`
---- 

The [hotspot=14-21 file=1]`buildscript` section defines plug-in versions to use in the build and 
where to find them. This guide uses the [hotspot=19 file=1]`liberty` plug-in, which is available 
from the Maven Central Repository.

[role="code_command hotspot=1-12", file=1, subs="quotes"] 
---- 
Add the `repositories` section to the build file.
`finish/build.gradle`
---- 

The [hotspot=23-25 file=1]`repositories` section defines where to find the dependencies that you are using in the build.
For this build, everything you need is in Maven Central.

[role="code_command hotspot=27-33", file=1, subs="quotes"] 
---- 
Add the `dependencies` section to the build file.
`/build.gradle[tag=**]`
---- 

The [hotspot=27-33 file=1]`dependencies` section defines what is needed to compile and 
test the code. This section also defines how to run the application. The [hotspot=28 file=1]`providedCompile` dependencies are APIs that are needed to compile the application, but they do not need to be packaged with the application because Open Liberty provides their implementation at run time.
For example, the `HelloServlet.java` class depends on [hotspot=28 file=1]`javax.servlet-api` to compile, but since Open Liberty provides that API, it does not need to be packaged with your application. The [hotspot=29-30 file=1]`testCompile` dependencies are needed to compile and run tests.

The [hotspot=42-54 file=1]`liberty` plug-in provides the [hotspot=31-32 file=1]`libertyRuntime` dependency definition that specifies the version of the Liberty runtime you want to use.
This project uses Open Liberty 17.0.0.4 or later.

[role="code_command hotspot=35-40, file=1, subs="quotes"] 
---- 
...
[role="code_command hotspot=42-54", subs="quotes"] 
---- 
...
| [hotspot=44 file=1]`name` | The server name.
| [hotspot=45 file=1]`configFile` | The server configuration file.
| [hotspot=46-48 file=1]`bootstrapProperties` | Bootstrap properties that are written to the server's `bootstrap.properties` file.
| [hotspot=49-52 file=1]`packageLiberty` | Specifies to only package the `usr` directory in the `build/GradleSample.zip` file.
|===

So what's not configured for the [hotspot=42-54 file=1]`liberty` plug-in?
By default, when the [hotspot42-54 file=1]`liberty` and the [hotspot=56-58 file=1]`war` plug-ins are applied, the default WAR file is installed to the `apps` folder. Omitting this definition is the same as including the following line:
...
[role="code_command hotspot=56-58", file=1, subs="quotes"] 
---- 
...
The [hotspot=56-58 file=1]`war` plug-in extends the https://docs.gradle.org/current/userguide/java_plugin.html[Java plug-in], which provides test tasks.
...
[role="code_command hotspot=10-34", subs="quotes"] 
---- 
...
[role="code_command hotspot=60-75", subs="quotes"] 
---- 
...
The [hotspot=74 file=1]`systemProperties` configuration defines some variables needed by the test class.
While the port number and context-root information can be hardcoded in the test class, it is better to specify it in a single place like the Gradle [hotspot=1-97 file=1]`build.gradle` file, in case they need to change.
The [hotspot=74 file=1]`systemProperties` section passes these details to the Java test program as a series of system properties, resolving the [hotspot=74 file=1]`liberty.text.port` and [hotspot=74 file=1]`war.name` variables.

The following lines in the [hotspot=15-17 file=0]`EndpointIT.java` test class uses these system variables to build the URL of the application:

In the test class, after defining how to build the application URL, the [hotspot=20 file=0]`@Test` annotation indicates the start of the test method.

In the [hotspot=24-32 file=0]`try` block of the test method, an HTTP `GET` request to the URL of the application returns a status code.
...
In the [hotspot=3-8 file=0]`import` statements of this test class, you'll notice that the test has some new dependencies.
Earlier you added some [hotspot=29-30 file=1]`testCompile` dependencies.
The Apache `commons-httpclient` and `junit` dependencies are needed to compile and run the integration test [hotspot=10-34 file=0]`EndpointIT` class.

The scope for each of the dependencies is set to [hotspot=29-30 file=1]`testCompile` because the libraries are needed only during the Gradle test phase and do not need to be packaged with the application.
...
== A few more pieces
We show a few more Gradle tricks in this example with the [hotspot=77-83]`openBrowser` and the [hotspot=85-90]`openTestReport` tasks.
These tasks display your application and the test report in the default browser.

[role="code_command hotspot=77-90", subs="quotes"] 
---- 
...
[role="code_command hotspot=92-87", subs="quotes"] 
---- 
...
Then rerun the Gradle integration tests by running the command:
