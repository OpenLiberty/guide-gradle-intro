...
:page-tags: ['Getting Started', 'Gradle']
...
The first section of code defines the [hotspot=1 file=1]`war` and [hotspot=2 file=1]`liberty` plug-ins 
...
The Gradle [hotspot=35-40 file=1]`extra properties` extension allows you to add properties to a Gradle project.
...
The [hotspot=42-54 file=1]`liberty` extension configures the Liberty server, which includes the following settings:
[cols="40, 100"]
|===
...
The [hotspot=56-58 file=1]`war` configuration names the WAR file `GradleSample.war` without a version in the name.
...
The [hotspot=60-75 file=1]`test` section in your build file configures Gradle to run the integration tests in the `it` folder.
...
The [hotspot=14-18 file=0]`init()` method in the `EndpointIT.java` test class uses these 
...
The [hotspot=92-97]`dependency directives` associate the `war` and `java` tasks to the `liberty` tasks to get proper 
task ordering.
