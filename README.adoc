...
One benefit of using a build tool like Gradle is that you can define the details of the project and any dependencies it has, and then Gradle automatically downloads and installs the dependencies.
...
Choosing a build tool often comes down to personal preference, but you might choose to use Gradle for several reasons. 
Gradle defines its builds using https://docs.gradle.org/current/userguide/writing_build_scripts.html[Groovy build scripts^], which gives you a lot of control and customization in your builds. 
Gradle also makes use of a build cache, only rebuilding parts of your application that are changed, which saves build time in larger projects. 
