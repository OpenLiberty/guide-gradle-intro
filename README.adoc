...
which is available from the [hotspot=buildscriptmaven file=1]`Maven Central Repository`.
...
[hotspot=junit file=1]`useJUnitPlatform()` line configures Gradle to add JUnit 5 support.
...
We show a few more Gradle tricks in this example with the [hotspot=openbrowser file=0]`openBrowser` task. 
...
The [hotspot=depends file=0]`dependency directives` organizes task execution. 
In this case, the test task is set to run after the server is started, and the
[hotspot=openbrowser file=0]`openBrowser` task is executed after the test task is finalized.
...
build command to automatically restart and retest your application:
