...
automated test, and configure Gradle to run it after building the application. This application
utilizes the Gradle Wrapper configuration. The Gradle Wrapper is a script that invokes a declared 
version of Gradle, downloading it beforehand if necessary. The Wrapper is also the recommended 
way to execute any Gradle build. 
...
You will create these project configurations one section at a time. The Gradle Wrapper
files will be included in the project's directories, in order to execute the Wrapper. 
`gradlew` is required for unix based systems, and `gradlew.bat` is required for windows.
...
You can now run the build command to build and test the application. The Gradle Wrapper will 
download gradle if necessary. 
