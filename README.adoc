...
[role="code_command hotspot=23-25 file=1", subs="quotes"] 
---- 
...
[role="code_command hotspot=42-54 file=1", subs="quotes"] 
---- 
...
[role="code_command hotspot=10-35", subs="quotes"] 
---- 
...
[role="code_command hotspot=60-75 file=1", subs="quotes"] 
---- 
...
In the [hotspot=24-33 file=0]`try` block of the test method, an HTTP `GET` request to the URL of the application returns a status code.
...
The Apache `commons-httpclient` and `junit` dependencies are needed to compile and run the integration test [hotspot=10-35 file=0]`EndpointIT` class.
